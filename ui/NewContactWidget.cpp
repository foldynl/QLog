...
void NewContactWidget::setDxccInfo(const DxccEntity &curr)
{
    ...
    if ( dxccEntity.dxcc )
    {
        ...
        ui->stationTableWidget->setCallsign(getCallsign(),BandPlan::freq2Band(ui->freqTXEdit->value())); //aa5sh
...
void NewContactWidget::updateTXBand(double freq)
{
    ...
    updateDxccStatus();
    ui->dxccTableWidget->setDxcc(dxccEntity.dxcc, BandPlan::freq2Band(ui->freqTXEdit->value()));
    ui->stationTableWidget->setCallsign(getCallsign(),BandPlan::freq2Band(ui->freqTXEdit->value()));
}
